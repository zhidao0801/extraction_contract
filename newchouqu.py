...
def read_word(filelist):
    doccontent = textract.process(filelist) # 提取word文档里面的内容
    return doccontent   
def extract_title(content):
    ...
    if re.search(r'\|(\s*?)合同名称(\s*?)\|',doccontent,re.S):   #判断word文档里面的内容是否是表格   
        ...
        print hetongmingcheng
    return hetongmingcheng
def extract_number(content): # 提取合同编号
    doccontent=content
    hetongbianhao=""
    if re.search(r'合同编号(.*?)(:|：)\n',doccontent,re.S):
...
def extract_money(content): # 提取合同金额
    hetongjiner=""
    doccontent=content 
    if re.search(r'人民币大写(.*?)）',re.sub(r'\n','',doccontent),re.S): 
        ...
        print hetongjiner
    return hetongjiner 
def extract_secondparty(content): # 提取乙方信息
    doccontent=content
    hetongduifang=""
    if re.search(r'受托方(.*?)(:|：)(.*?)\n',doccontent,re.S):
...
def extract_undertakingdepartment(content): # 提取承办部门信息
    doccontent=content
    a=doccontent.split('\n')
    chengbanbumen=""
...
def extract_dateofsigning(content): # 提取签订日期
    qiandingrqi=""
    doccontent=content
...
def extract_dateprepared(content):  
    ...
    a=doccontent.split('\n')
    dateprepared=""
    if re.search(r'\|(.*?)拟稿时间(.*?)\|',doccontent,re.M):  # 提取word表格里面的拟稿时间
        for i in range(len(a)):
            if "拟稿时间" in a[i]:
                for q in range(len(a[i].split('|'))):
...
def main(rootdir,path):
    filelists=getpathlist(rootdir)  # 获得rootdir 下所有的文件
    result_all=extract_all(filelists) #获得 抽取的所有内容
    insert_table(path,result_all) # 将获得的内容插入表格中
